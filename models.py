...
class BlurPool(nn.Module):
    def __init__(self, in_channels, size, padding):
        ...
        self.register_buffer('W', w[None, None,:,:].repeat((self.in_channels, 1, 1, 1)))
        ...
        self.pad = PadLayer([int((size-1)/2), int((size-1)/2), int((size-1)/2), int((size-1)/2)])
    def forward(self, x):
...
class ResNet9(nn.Module):
    def __init__(self, in_ch, in_dim, c_num):
        ...
        self.linear = nn.Sequential(nn.MaxPool2d(3),
                                        nn.Flatten(),
                                        nn.Linear(512, c_num))    # num_cls
